...
.controller('MenuCtrl', ['$scope', '$location', '$resource', '$q', 'data', '$window', function ($scope, $location, $resource, $q, data, $window) {
	...
	$scope.showMenu = function() {
		...
		if (parseFloat(window.device.version) >= parseFloat(3.0)) {
			topMenuRightHTML = angular.element($window.document.getElementById('topMenu-right'));
            topMenuRightHTML.css('bottom', '0px');  
        }
	}
}])
.controller('LibraryCtrl', ['$scope', '$rootScope', '$location', '$timeout', '$resource', '$q', 'data', '$window', function ($scope, $rootScope, $location, $timeout, $resource, $q, data, $window) {
    ...
    $scope.showDocumentInfo = function () {
		...
		if (parseFloat(window.device.version) >= parseFloat(3.0)) {
            documentInfoHTML = angular.element($window.document.getElementById('info-container'));
            documentInfoHTML.css('bottom', '0px'); 
        }
    }
